...
object Inline {
  ...
  transparent inline def g(x: Matchable): Any = inline x match
  ...
  transparent inline def defaultValue[T <: Matchable] = inline erasedValue[T] match
  ...
  inline def setFor2[T]: Set[T] = summonFrom {
  ...
  inline def f: Any = summonFrom {
    case given A1 => ???  // error: ambiguous implicits
  }
